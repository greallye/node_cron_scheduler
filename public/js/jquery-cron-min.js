(function(e) {
    ...
    var x = {
        init: function(i) {
            ...
            C.period = e("<span class='cron-period'>Every <select style='width:75px;font-size:11px;' name='cron-period'>" + A + r + "</select> </span>").appendTo(this).data("root", this);
            var E = C.period.find("select");
            E.bind("change.cron", m.periodChanged).data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(j)
            }
            C.dom = e("<span class='cron-block cron-block-dom'> on the <select style='width:75px;font-size:11px;' name='cron-dom'>" + v + "</select> </span>").appendTo(this).data("root", this);
            E = C.dom.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.domOpts)
            }
            C.month = e("<span class='cron-block cron-block-month'> of <select style='width:75px;font-size:11px;' name='cron-month'>" + h + "</select> </span>").appendTo(this).data("root", this);
            E = C.month.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.monthOpts)
            }
            C.mins = e("<span class='cron-block cron-block-mins'> at <select style='width:75px;font-size:11px;' name='cron-mins'>" + y + "</select> minutes past the hour </span>").appendTo(this).data("root", this);
            E = C.mins.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.minuteOpts)
            }
            C.dow = e("<span class='cron-block cron-block-dow'> on <select style='width:75px;font-size:11px;' name='cron-dow'>" + s + "</select> </span>").appendTo(this).data("root", this);
            E = C.dow.find("select").data("root", this);
            if (B.useGentleSelect) {
                E.gentleSelect(B.dowOpts)
            }
            C.time = e("<span class='cron-block cron-block-time'> at <select style='width:75px;font-size:11px;' name='cron-time-hour' class='cron-time-hour'>" + d + "</select>:<select name='cron-time-min' class='cron-time-min'>" + y + " </span>").appendTo(this).data("root", this);
